global alpha gamma epsilon beta kappa K;
alpha = .2205; %
beta = 4.2801; %
gamma = .365; %
...
function dU = dU_dt(U, V)
    global alpha gamma kappa K
    dU = (alpha * U * (1 - U / K)) - ((gamma * U) / (1 + gamma * kappa * U)) * V;
end
...
Ut = zeros(1, num_steps + 1);
Vt = zeros(1, num_steps + 1);
...
V_cline = (alpha / gamma) * (1 - ((1 - gamma * kappa * K) / K) * U_cline - (gamma * kappa / K) * U_cline^2); % Hare nullcline (V*)
